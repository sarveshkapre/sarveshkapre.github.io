...
async function fetchWebmentions(timeFrom: string | null, perPage = 1000) {
	if (!DOMAIN) {
		if (import.meta.env.DEV) console.warn("No domain specified. Please set in astro.config.ts");
		return null;
	}

	if (!WEBMENTION_API_KEY) {
		if (import.meta.env.DEV) console.warn("No webmention api token specified in .env");
		return null;
	}
