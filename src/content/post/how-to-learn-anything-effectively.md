...
The “why” arrived later in the real world, when code got longer, messier, and more interconnected. Abstraction stops being theory and becomes how you keep things sane: you pull repeated logic into one place (a single source of truth), so a small change doesn’t ripple everywhere, and stop doing naive loops once performance starts to hurt (like switching from nested loops to a hash map and watching something go from “fine at small scale” to actually fast). And that’s when algorithms become real too, ironically, you often reinvent the right one just by thinking through the logic, long before you remember what the textbook called it.
